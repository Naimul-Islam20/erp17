"use client";

import React from "react";
import ProductImage from "@/components/ui/ProductImage";
import { AiOutlineCheckCircle } from "react-icons/ai";

const posSections = [
  {
    heading: "Fast checkout",
    subText:
      "Built for busy counters: barcode scan, weighed items, modifiers, and split payments—keyboard shortcuts for power cashiers.",
    list: [
      "Hardware support for common scanners and receipt printers.",
      "Hold / recall bills when a customer steps away.",
      "Discount rules by role so only supervisors can override price.",
    ],
    img: "/img/products/point-of-sale/register.svg",
    imgRight: false,
  },
  {
    heading: "Registers & shifts",
    subText:
      "Open and close shifts with blind counts; every till ties to a user and branch so HQ sees variance in real time.",
    list: [
      "Multi-register stores with consolidated day-end report.",
      "Cash pickup and mid-shift drops logged with manager PIN.",
      "Works online; queues sales locally if the link drops briefly.",
    ],
    img: "/img/products/point-of-sale/receipt.svg",
    imgRight: true,
  },
  {
    heading: "Receipts & returns",
    subText:
      "Digital or printed receipts with return policy text; look up any sale by receipt number, phone, or card token.",
    list: [
      "Return against original transaction with automatic stock put-back.",
      "Exchange flows that preserve margin and tax lines.",
      "Customer display option for item names and running total.",
    ],
    img: "/img/products/point-of-sale/register.svg",
    imgRight: false,
  },
];

const PointOfSalePage = () => {
  return (
    <main className="bg-white">
      <section className="relative overflow-hidden bg-emerald-50 pt-10 md:pt-14 pb-10 md:pb-20">
        <div className="absolute top-0 right-0 w-1/2 h-full bg-emerald-100/60 -skew-x-12 transform origin-top-right translate-x-1/4 hidden md:block" />
        <div
          className="absolute top-10 left-10 w-32 h-32 opacity-20 hidden md:block"
          style={{
            backgroundImage: "radial-gradient(#059669 1.5px, transparent 1.5px)",
            backgroundSize: "15px 15px",
          }}
        />
        <div className="absolute -top-20 -left-20 w-64 h-64 bg-emerald-200/40 rounded-full blur-3xl animate-pulse" />
        <div className="absolute top-1/2 -right-20 w-80 h-80 bg-teal-200/30 rounded-full blur-3xl" />

        <div className="absolute top-0 right-0 p-8 opacity-10 hidden lg:block">
          <svg width="200" height="200" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              d="M0 100C0 44.7715 44.7715 0 100 0C155.228 0 200 44.7715 200 100C200 155.228 155.228 200 100 200C44.7715 200 0 155.228 0 100Z"
              fill="url(#paint0_linear_pos)"
            />
            <defs>
              <linearGradient id="paint0_linear_pos" x1="100" y1="0" x2="100" y2="200" gradientUnits="userSpaceOnUse">
                <stop stopColor="#059669" />
                <stop offset="1" stopColor="#059669" stopOpacity="0" />
              </linearGradient>
            </defs>
          </svg>
        </div>

        <div className="container relative z-10">
          <div className="flex flex-col md:flex-row items-center gap-10 md:gap-16">
            <div className="flex-1 space-y-6 text-center md:text-left">
              <div className="inline-flex items-center px-3 py-1 rounded-full bg-emerald-100 text-emerald-800 text-xs md:text-sm font-bold uppercase tracking-wider">
                ERP17 · Point of Sale
              </div>
              <h1 className="text-4xl sm:text-5xl md:text-7xl font-extrabold text-gray-900 leading-[1.1] tracking-tight">
                Retail speed <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-teal-500">
                  with HQ control
                </span>
              </h1>
              <p className="text-gray-600 text-base md:text-xl leading-relaxed max-w-2xl mx-auto md:mx-0">
                Run tills that match how your shops actually work—then feed sales, stock, and cash data straight into ERP17 without
                re-keying at night.
              </p>
              <div className="flex flex-wrap justify-center md:justify-start gap-4 pt-4">
                <button
                  type="button"
                  className="px-8 py-4 bg-emerald-600 text-white font-bold rounded-2xl hover:bg-emerald-700 shadow-xl shadow-emerald-200 transition-all hover:scale-105 active:scale-95 text-sm md:text-base"
                >
                  Contact Us
                </button>
                <button
                  type="button"
                  className="px-8 py-4 bg-white text-emerald-700 font-bold rounded-2xl border-2 border-emerald-100 hover:bg-emerald-50 transition-all text-sm md:text-base"
                >
                  View Demo
                </button>
              </div>
            </div>
            <div className="flex-1 w-full relative">
              <ProductImage
                src="/img/products/point-of-sale/hero.svg"
                alt="Point of sale receipt preview"
                width={600}
                height={400}
                className="w-full h-auto rounded-2xl"
                priority
              />
            </div>
          </div>
        </div>
      </section>

      <section className="py-20 bg-white">
        <div className="container space-y-24 md:space-y-32">
          {posSections.map((sec, idx) => (
            <div key={idx} className="flex flex-col md:flex-row items-center gap-12 md:gap-20">
              <div
                className={`flex-1 space-y-6 text-center md:text-left order-2 ${
                  sec.imgRight ? "md:order-1" : "md:order-2"
                }`}
              >
                <h2 className="text-3xl md:text-4xl font-bold text-gray-900 leading-tight">{sec.heading}</h2>
                <p className="text-gray-600 text-base md:text-lg leading-relaxed">{sec.subText}</p>
                <div className="grid gap-4 pt-2">
                  {sec.list.map((item, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-4 bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow"
                    >
                      <div className="bg-emerald-50 p-1 rounded-full flex-shrink-0 mt-1">
                        <AiOutlineCheckCircle className="text-emerald-600 w-5 h-5 md:w-6 md:h-6" />
                      </div>
                      <p className="text-gray-700 text-sm md:text-base font-medium">{item}</p>
                    </div>
                  ))}
                </div>
              </div>
              <div className={`flex-1 w-full order-1 ${sec.imgRight ? "md:order-2" : "md:order-1"}`}>
                <ProductImage src={sec.img} alt={sec.heading} width={500} height={400} className="w-full h-auto rounded-2xl" />
              </div>
            </div>
          ))}
        </div>
      </section>
    </main>
  );
};

export default PointOfSalePage;
